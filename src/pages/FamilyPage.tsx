  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      const newPhoto: FamilyPhoto = {
        ...
        caption: caption || t('A precious memory'),
  ...
  return (
    <div className="min-h-screen px-4 pt-20 pb-8 max-w-3xl mx-auto">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-charcoal-800 dark:text-white">{t('My Family')}</h1>
          <p className="text-charcoal-400 dark:text-charcoal-500 text-sm">{t('Photos and memories from your loved ones')}</p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-2xl bg-white/60 dark:bg-white/10 backdrop-blur-xl border border-white/50 dark:border-white/10 mb-8">
        <div className="flex items-center gap-3 mb-4">
          ...
          <h2 className="font-semibold text-charcoal-800 dark:text-white">{t('Add a Photo')}</h2>
        </div>
        <input type="text" value={caption} onChange={e => setCaption(e.target.value)}
          placeholder={t("Add a caption (e.g., 'My daughter Priya')")}
      ...
      {photos.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-charcoal-400 dark:text-charcoal-500">{t('No family photos yet. Upload one above!')}</p>
          <p className="text-charcoal-300 dark:text-charcoal-600 text-sm mt-2">{t('Photos you add will appear in your memory games too.')}</p>
        </div>
      ) : (
      ...
      <div className="text-center mt-12 p-6 rounded-2xl bg-white/30 dark:bg-white/5 backdrop-blur-sm border border-white/30 dark:border-white/10">
        ...
        <p className="text-charcoal-500 dark:text-charcoal-400 text-sm italic">{t('Family photos help your memory games feel personal and familiar.')}</p>
      </div>
    </div>
  )
}
